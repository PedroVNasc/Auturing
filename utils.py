...
class Node(QGraphicsItem):
    def __init__(self, center: QPointF, text: str):
        ...
        self.text_child = EditableText(text, center)
...
class Arrow(QGraphicsLineItem):
    def __init__(self, source: Node, target: Node, data: str):
        ...
        self.source, self.target, self.angle = self.calculateLine(source, target)
        self.data = data
...
class GraphHandler(QGraphicsWidget):
    ...
    def updateConnection(self, node: Node, data: str = ''):
        connections = self.getConnections(node) + self.getConnections(node, target=True)
        
        for conn in connections:
            conn['line'].updateLine(conn['source_node'], conn['target_node'], data)
